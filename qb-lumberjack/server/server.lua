...
RegisterNetEvent('qb-lumberjack:sellItems', function()
    local source = source
    local price = 0
...
RegisterNetEvent('qb-lumberjack:BuyAxe', function()
    local source = source
...
QBCore.Functions.CreateCallback('qb-lumberjack:axe', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player ~= nil then
...
RegisterNetEvent('qb-lumberjack:setLumberStage', function(stage, state, k)
    Config.TreeLocations[k][stage] = state
    TriggerClientEvent('qb-lumberjack:getLumberStage', -1, stage, state, k)
end)

RegisterNetEvent('qb-lumberjack:setChoppedTimer', function()
    if not Chopped then
        Chopped = true
        CreateThread(function()
            ...
            for k, v in pairs(Config.TreeLocations) do
                ...
                TriggerClientEvent('qb-lumberjack:getLumberStage', -1, 'isChopped', false, k)
            end
            Chopped = false
        end)
    end
...
RegisterServerEvent('qb-lumberjack:recivelumber', function()
    local source = source
    local Player = QBCore.Functions.GetPlayer(tonumber(source))
...
QBCore.Functions.CreateCallback('qb-lumberjack:lumber', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player ~= nil then
...
RegisterServerEvent('qb-lumberjack:lumberprocessed', function()
    local source = source
    local Player = QBCore.Functions.GetPlayer(tonumber(source))
